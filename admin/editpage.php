...
if(isset($_GET['pid'])&&$_GET['pid']!=''){
    ...
    $cities_array=explode (",", $fetch['related_cities']); 
...
if(isset($_POST['addpage'])){
  ...
  $state=implode(',',$st);
		if($_FILES['image2']['name']!='') {
			$excQry=mysqli_query($con,"UPDATE `websitepage` SET `related_cities`='$state', `city`='$city',`url`='$url',`content`='$content',`image`='$filename',`img_alt`='$imgalt',`def`='$content_chk',`contact`='$contact',`email`='$email',`status`='1',`date`='$date' where `id`='$id'");
		}else{
			$excQry=mysqli_query($con,"UPDATE `websitepage` SET `related_cities`='$state', `city`='$city',`url`='$url',`content`='$content',`contact`='$contact',`def`='$content_chk',`img_alt`='Escort',`email`='$email',`status`='1',`date`='$date' where `id`='$id'");
...
<div class="row">
	...
	<label for="slug">Contact <span style="color:red">*</span> </label> <label for="contact_chk" style="float:right;text-align:right"> Default <input type="checkbox" placeholder="contact"  id="contact_chk" name="contact_chk" style="margin-left:10px"></label>
  ...
  <div class="row">
    ...
    <?php $cityqry=mysqli_query($con,"select id,city from websitepage order by id desc");
        while($cityfetch=mysqli_fetch_array($cityqry)){?>
         <div class="col-md-2">
           <label for="<?php echo $cityfetch['id'] ?>" > 
  	 <input type="checkbox" placeholder="contact" class="form-checkbox" value="<?php echo $cityfetch['id'] ?>" <?php  if (in_array($cityfetch['id'], $cities_array)){ ?> checked <?php } ?>  id="<?php echo $cityfetch['id'] ?>" name="cities[]" > <?php echo $cityfetch['city'] ?>
  </label>
         </div>
    <?php }?>
  </div></div>
...
<h4 class="card-title mb-4">Page Content
   <label for="content_chk" style="float:right;text-align:right"> 
  	Default Page Content &nbsp;&nbsp;<input type="checkbox" placeholder="contact" class="form-checkbox" value="1" <?php if($fetch['def']=='1'){ ?> checked <?php } ?>  id="content_chk" name="content_chk" >
  </label>
	 </h4>

	<div class="row" id="default_content_div" <?php if($fetch['def']=='1'){?> style=""<?php }else{?>style="display:none" <?php } ?> >
...
<div class="row"  id="edit_content_div" <?php if($fetch['def']=='0'){?> style="display:block"<?php }else{?>style="display:none" <?php } ?>>
<div  class="mb-3 col-md-8">
  <div class="row">
    <div class="col-md-12 mb-3">
	<label for="name">Content <span style="color:red">*</span></label><textarea id="editor" name="content" class="form-control"><?php echo $fetch['content'] ?></textarea>
	</div>
...
<div  class="mb-3 col-md-3">
    <label for="name">Image <span style="color:red">*</span></label>
	<br><img id="pimage" style="max-width: 100%;" src=""  alt="">
</div>
</div>
